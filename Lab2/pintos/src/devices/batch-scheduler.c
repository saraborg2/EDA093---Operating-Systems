...
static struct condition cond[NUM_OF_DIRECTIONS][NUM_OF_PRIORITIES];
static int current_task_direction;
static int tasks_on_bus;
static int waiting_tasks[NUM_OF_DIRECTIONS][NUM_OF_PRIORITIES];
...
void get_slot (const task_t *task) {
  ...
  direction_t other_dir = (direction==SEND) ? RECEIVE : SEND;

  lock_acquire(&bus_lock);
  while((tasks_on_bus == BUS_CAPACITY) || 
        (tasks_on_bus > 0 && task->direction != current_task_direction ) || 
        (priority == NORMAL && (waiting_tasks[direction][PRIORITY] > 0 || waiting_tasks[other_dir][PRIORITY] > 0))){
      
      waiting_tasks[direction][priority]++;
      cond_wait(&cond[direction][priority], &bus_lock);
      waiting_tasks[direction][priority]--;
  }

  tasks_on_bus++;
  current_task_direction = direction;
...
void release_slot (const task_t *task) {
  ...
  if (tasks_on_bus == 0){
    if (waiting_tasks[direction][PRIORITY] > 0){
      cond_broadcast(&cond[direction][PRIORITY], &bus_lock);
    }
    else if (waiting_tasks[other_dir][PRIORITY] > 0){
      cond_broadcast(&cond[other_dir][PRIORITY], &bus_lock);
    }
    else if (waiting_tasks[direction][NORMAL] > 0){
      cond_broadcast(&cond[direction][NORMAL], &bus_lock);
    }
    else if (waiting_tasks[other_dir][NORMAL] > 0){
      cond_broadcast(&cond[other_dir][NORMAL], &bus_lock);
    }
  } 
  else {
    if (waiting_tasks[direction][PRIORITY]){
      cond_signal(&cond[direction][PRIORITY], &bus_lock);
    }
    else if (waiting_tasks[other_dir][PRIORITY] == 0){
      cond_signal(&cond[direction][NORMAL], &bus_lock);
    }
  }
  lock_release(&bus_lock);
}
